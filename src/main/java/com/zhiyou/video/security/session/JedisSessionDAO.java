package com.zhiyou.video.security.session;
...
import com.zhiyou.video.util.DateUtils;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	protected Serializable doCreate(Session session) {
		...
		if (request != null){
			String uri = request.getServletPath();
			// 如果是静态文件，则不创建SESSION
			if (Servlets.isStaticFile(uri)){ 	//对静态资源的访问，不进行session的
		        return null;
			}
		}
		//生产sessionid
		Serializable sessionId = this.generateSessionId(session);
		this.assignSessionId(session, sessionId);//sessionid和session一一对应。
		this.update(session);
		return sessionId;
	}
